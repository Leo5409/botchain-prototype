...
管理后台是 A2 节点 DAPP 的**运营中枢**：C 端的所有可变内容（阶段、价格、能量倍数、三类奖励比例、收款地址）都在这里配置，所有资金动作（入金核对、提币审核打款）都在这里落地。
...
| 节点与参数 | **F-05 节点参数**      | 档位原价 / 能量倍数 / **节点、直推、团队三类奖励比例** / 启用停用 + 规则变更记录     |
...
| 档位   | 原价       | 能量倍数  | 节点奖励  | 直推奖励  | 团队奖励  |
| ---- | -------- | ----- | ----- | ----- | ----- |
| A 节点 | 1,000 U  | 1.5 × | 1.00% | 5.00% | 2.00% |
| B 节点 | 5,000 U  | 2.0 × | 1.50% | 5.00% | 4.00% |
| C 节点 | 10,000 U | 2.5 × | 2.00% | 5.00% | 6.00% |
| D 节点 | 30,000 U | 3.0 × | 3.00% | 5.00% | 8.00% |
...
- 所有高危配置（收款地址、节点原价、能量倍数、三类奖励比例、阶段名额）修改需填写原因，并记录**修改前 / 修改后值、操作人、时间、IP**。
...
**档位表**（9 列）：节点等级 / 说明 / 原价(U) / 能量倍数 / **节点奖励比例** / **直推奖励比例** / **团队奖励比例** / 状态 / 最后修改 / 操作
...
| **安全设置**   | Google 验证器（已绑定状态 + 重置入口）/ 登录保护策略（只读）/ 界面语言                                             |
...
| 参数                                   | 归属页面     | 可操作角色           |
| ------------------------------------ | -------- | --------------- |
| 节点原价 / 能量倍数 / **三类奖励比例** / 启用停用      | 节点参数     | 系统管理员、超级管理员     |
